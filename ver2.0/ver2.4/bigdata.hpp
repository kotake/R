...
class Bigdata{
	public:
		vector<all_data::kmeans_data> kmeans(CSVData mycsv);
		void gnuplot(CSVData mycsv, int sheet_num, int rab_x, int rab_y);
		vector<all_data::sokan_data> sokan(CSVData mycsv);
		vector<all_data::aso_data> aso(CSVData mycsv);
...
void all_data::sokan_data::set(string r_x, string r_y, double vr){
	rab_x = r_x;
	rab_y = r_y;
	r = vr;
}

vector<all_data::sokan_data> Bigdata::sokan(CSVData mycsv){
	...
	vector<all_data::sokan_data> sokan;//返り値の宣言
	all_data::sokan_data s;//これsをsokanにnC2回push_backする
...
void all_data::kmeans_data::set(string r_x, string r_y){
	rab_x = r_x;
	rab_y = r_y;
}

all_data::kmeans_data::point all_data::kmeans_data::set(double vx, double vy, int vc){
	point p;
	p.x = vx;
	p.y = vy;
	p.cluster = vc;
...
//エントリポイント
vector<all_data::kmeans_data> Bigdata::kmeans(CSVData mycsv){
...
int all_data::aso_data::get_col(ifstream &ifs){
...
void all_data::aso_data::to_var(ifstream &ifs){
	string s, tmp;
...
vector<all_data::aso_data> Bigdata::aso(CSVData mycsv){
	...
	vector<all_data::aso_data> aso;//返り値宣言
	all_data::aso_data a;
